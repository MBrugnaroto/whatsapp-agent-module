...
class MemoryManager:
    ...
    async def extract_and_store_memories(self, message: BaseMessage, thread_id: str) -> None:
        ...
        if analysis.is_important and analysis.formatted_memory:
            similar = self.vector_store.find_similar_memory(analysis.formatted_memory, thread_id=thread_id)
            if similar:
    ...
    def get_relevant_memories(self, context: str, thread_id: str) -> List[str]:
        """Retrieve relevant memories based on the current context."""
        memories = self.vector_store.search_memories(context, k=settings.MEMORY_TOP_K, thread_id=thread_id)
        if memories:
